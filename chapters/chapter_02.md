...
First, we need to load the package {gapminder} with the function library to obtain the `gapminder` data frame object. Then, we can have a look at the `gapminder` object structure using the function `str()` as well as at its first rows/observations using the function `head()`:
...
<exercise id="11" title="The pipe operator %>%" type="slides">

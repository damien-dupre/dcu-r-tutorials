...
R Scripts contains the brute code and some comments, that's it. Even though these files can look very raw, they are usually enough to ensure the reproducibility of your analysis, for you at least.
...
Then some specific options are used. There is an infinite number of options to modify but the ones that I find the most useful are the following:
...
It is also very easy to create your own functions in R. The best is to store them at the beginning of a script if they are short or to name a R script dedicated to these home-made functions using `source("C:/path/to/script/function.R")`.
...
The main argument of these functions is `file` which corresponds to the path to a file, followed by the name of the file and its extension type such as **C:/path/to/my/file.csv** which will be open in R with the following code:
...
To access data, an even easier way would be to use a website URL for data stored online instead of file paths.

Base R provides basic functions to open a data file, but there are also foreign packages that are providing functions to open data files as well such as {readr}, {data.table} or {vroom} for example.

Because we have started in the tidyverse, let's stay in the tidyverse and use the [{readr}](https://readr.tidyverse.org/) and [{readxl}](https://readxl.tidyverse.org/) packages (even if they are not the most efficient ones).

Usually, .xls and .xlsx file formats MUST BE AVOIDED and .csv file formats used instead. The reason is because .csv files are open-source, light weight, and clean whereas .xls and .xlsx do not have any of these essential properties.
